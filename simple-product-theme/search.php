...
<section class="container mb-5">
    <?php if ( have_posts() ) : ?>
        <h1 class="fa-300 text-center pb-5">Search results for : <?php echo get_search_query(); ?></h1>
        <?php while ( have_posts() ) : ?>
            <div class="row">
                <div class="col-md-12">
                    <?php the_post(); ?>
                    <?php the_title( sprintf( '<h4 class="fa-500"><a href="%s" rel="bookmark">', esc_url( get_permalink() ) ), '</a></h4>' ); ?>
                    <?php the_excerpt(); ?>
                </div>
            </div>
        <?php endwhile; ?>
    <?php else :?>
        <h1 class="fa-300 text-center pb-5">No Match Found for : <?php echo get_search_query(); ?></h1>
    <?php endif;?>
</section>
<div class="clearfix"></div>
<?php
